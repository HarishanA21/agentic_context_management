...
- [x] **Strategy switch** — `DEFAULT_CONTEXT_STRATEGY` env var + `ChatRequest.context_strategy` per-request override + `GET /context/strategies` introspection endpoint. `_get_agent_for_request` branches on the strategy and the agent cache key includes it.
- [x] **TypeScript Code Mode** (`ts_code_mode`) — three-element on-demand design inspired by [Cloudflare's Code Mode](https://blog.cloudflare.com/code-mode/). System prompt carries a compact catalog (~30 tokens per tool); the agent calls `describe_tools(names)` to fetch the full TS interfaces + JSDoc for just the tools it needs; then `execute_typescript(code)` runs one TS program per turn inside a **Deno isolate** with `--allow-net=127.0.0.1:<port>` only — no fs, no subprocess, no other network. Tool invocations from the program route back through a per-call loopback HTTP server gated by a 32-byte token; described names are sticky for the rest of the LangGraph thread, so follow-up turns skip the describe step. Backend pieces: [backend/ts_code_mode.py](backend/ts_code_mode.py), [backend/ts_code_mode_rpc.py](backend/ts_code_mode_rpc.py), [backend/ts_code_mode_runner.py](backend/ts_code_mode_runner.py), [backend/ts_code_mode_registry.py](backend/ts_code_mode_registry.py), tool wrappers under `backend/Tools/`. Design notes in [TS_CODE_MODE_PLAN.md](TS_CODE_MODE_PLAN.md). Default stays `tool_calling`; opt in via env var or per-request field.
